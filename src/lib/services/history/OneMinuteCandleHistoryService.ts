...
export class OneMinuteCandleHistoryService {
  ...
  public generateOneMinuteCandleHistory = async (
    symbol: string
  ): Promise<string> => {
    const candles: Candle[] = await this.binanceService.getCandles(symbol, "1m", RECENT_CANDLES);
    let markdown = "";
    ...
    for (let index = 0; index < candles.length; index++) {
      ...
      const formattedTime = new Date(candle.closeTime).toISOString();
